...
def get_intersection_points(a: Line, b: Line) -> List[Point]:
    a_points = get_points_of_line(line=a)
    b_points = get_points_of_line(line=b)

    return set(a_points).intersection(set(b_points))

# should be tested
def get_points_of_line(line: Line) -> List[Point]:
    if line.start.x == line.end.x:
        return [Point(x=line.start.x, y=y) for y in range(min(line.start.y, line.end.y), max(line.start.y, line.end.y) + 1)]
    elif line.start.y == line.end.y:
        return [Point(x=x, y=line.start.y) for x in range(min(line.start.x, line.end.x), max(line.start.x, line.end.x) + 1)]
    else:
        # diag
        points = []
        x_f = -1 if line.start.x > line.end.x else 1
        y_f = -1 if line.start.y > line.end.y else 1 
        for i in range(0, max(line.start.x, line.end.x) - min(line.start.x, line.end.x) + 1):
            points.append(Point(x=line.start.x + x_f*i , y= line.start.y + y_f*i))
        return points
