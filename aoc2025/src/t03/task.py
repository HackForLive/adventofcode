...
def get_largest_number(bank: list[int], n: int) -> int:
    # n - length of number
    ll = len(bank)
    if ll < n:
...
def solve_1(p: Path):
    with open(p, 'r', encoding='utf8') as f:
        return functools.reduce(
            lambda a, b: a + b, [get_largest_number(bank=[int(i) for i in l.strip()], n=2) for l in f])

@timer_decorator
def solve_2(p: Path):
    with open(p, 'r', encoding='utf8') as f:
        return functools.reduce(
            lambda a, b: a + b, [get_largest_number(bank=[int(i) for i in l.strip()], n=12) for l in f])
