...
// How to build
//
//  g++ -o adv_2022_15 adv_2022_15_2.cpp -I /path_to/boost_1_80_0
//
...
int getResult(vector<vector<int>>& s, vector<vector<int>>& b, int &y);
...
bool isIntervalOverlapping(vector<int>& A, vector<int>& B){
    if(abs(A[1]-B[0]) == 1) return true;
    if(abs(A[0]-B[1]) == 1) return true;
